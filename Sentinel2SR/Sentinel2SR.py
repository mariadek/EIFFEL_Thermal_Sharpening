...
def run_Sentinel2_SR(input, output_file):
    ...
    model_2X.load_weights(os.path.join("Sentinel2SR","Best_2X.h5"))

    model_6X = s2model(input_shape_6X, num_layers = 6, feature_size = 128)

    model_6X.load_weights(os.path.join("Sentinel2SR","Best_6X_v2.h5"))
...
def run_Sentinel2_resampling(input, output_file):
    ...
    driver = gdal.GetDriverByName('GTiff')
    outdata = driver.Create(output_file, ds60.RasterXSize, ds60.RasterXSize, 12, gdal.GDT_UInt16)
    outdata.SetGeoTransform(ds60.GetGeoTransform())
    outdata.SetProjection(ds60.GetProjection())
    ...
    data10 = ds10.ReadAsArray()[validated_10m_indices, :, :]

    data20 = ds20.ReadAsArray()[validated_20m_indices, :, :]

    data60 = ds60.ReadAsArray()[validated_60m_indices, :, :]

    data10_ = np.zeros([data10.shape[0], data60.shape[1], data60.shape[2]])
    for k in range(data10.shape[0]):
        data10_[k] = scipy.ndimage.zoom(data10[k], 1/6, order=1)

    data20_ = np.zeros([data20.shape[0], data60.shape[1], data60.shape[2]])
    for k in range(data20.shape[0]):
        data20_[k] = scipy.ndimage.zoom(data20[k], 1/3, order=1)


    total = np.concatenate([data10_, data20_, data60], axis = 0)

    outdata.WriteArray(total)
    outdata.FlushCache()
